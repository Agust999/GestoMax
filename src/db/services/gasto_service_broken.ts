...
export class GastoService {
    ...
    try {
      ...
      if (periodo === "hoy") {
        ...
        const querySemana = `SELECT COALESCE(SUM(total_venta), 0) as ventas_semana
           FROM Venta v
           WHERE v.punto_id = ? 
           AND DATE(v.creado_en) >= DATE('now', '-7 days', 'localtime') 
           AND DATE(v.creado_en) <= DATE('now', 'localtime')
           AND v.creado_en IS NOT NULL`;

        console.log(`🔍 DEBUG Query Semana: ${querySemana}`);
        console.log(`🔍 DEBUG Params Semana: [${puntoId}]`);
        ...
        const queryMes = `SELECT COALESCE(SUM(total_venta), 0) as ventas_mes
           FROM Venta v
           WHERE v.punto_id = ? 
           AND strftime('%Y-%m', v.creado_en) = strftime('%Y-%m', 'now', 'localtime')
           AND DATE(v.creado_en) <= DATE('now', 'localtime')
           AND v.creado_en IS NOT NULL`;

        console.log(`🔍 DEBUG Query Mes: ${queryMes}`);
        console.log(`🔍 DEBUG Params Mes: [${puntoId}]`);

        const ventasMes = await getSingleValueFix<number>(queryMes, [puntoId]);
        ...
        console.log(
          `🔍 DEBUG Params Período: [${puntoId}, ${fechaInicio}, ${fechaFin}]`,
        );
    ...
    try {
      ...
      switch (periodo) {
        case "hoy":
          ...
          query += ` AND (categoria = 'General' AND DATE(fecha_gasto) = DATE('now', 'localtime')) OR (categoria = 'Salario' AND punto_id = ? AND activo = 1)`;
          params.push(puntoId);
          break;
        case "semana":
          // Para semana: gastos generales de la semana y salarios de trabajadores que han trabajado esta semana
          query += ` AND (categoria = 'General' AND fecha_gasto >= DATE('now', '-7 days', 'localtime') AND fecha_gasto <= DATE('now', 'localtime')) OR (categoria = 'Salario' AND punto_id = ? AND activo = 1)`;
          params.push(puntoId);
          break;
        case "mes":
          // Para mes: todos los gastos del mes actual (lógica existente que funciona)
          query += ` AND strftime('%Y-%m', fecha_gasto) = strftime('%Y-%m', 'now', 'localtime')`;
          break;
        case "periodo":
  ...
  static async obtenerVentasTrabajadorPeriodo(
    ...
    try {
      ...
      switch (periodo) {
        case "hoy":
          query += ` AND DATE(v.creado_en) = DATE('now', 'localtime')`;
          break;
        case "semana":
          query += ` AND DATE(v.creado_en) >= DATE('now', '-7 days', 'localtime') AND DATE(v.creado_en) <= DATE('now', 'localtime')`;
          break;
        case "mes":
          query += ` AND strftime('%Y-%m', v.creado_en) = strftime('%Y-%m', 'now', 'localtime')`;
          break;
        case "periodo":
          if (fechaInicio && fechaFin) {
  ...
  static async obtenerConsumosPropiosPeriodo(
    ...
    try {
      ...
      switch (periodo) {
        case "hoy":
          query += ` AND DATE(fecha_consumo) = DATE('now', 'localtime')`;
          break;
        case "semana":
          query += ` AND fecha_consumo >= DATE('now', '-7 days', 'localtime') AND fecha_consumo <= DATE('now', 'localtime')`;
          break;
        case "mes":
          query += ` AND strftime('%Y-%m', fecha_consumo) = strftime('%Y-%m', 'now', 'localtime')`;
          break;
        case "periodo":
          if (fechaInicio && fechaFin) {
  ...
  static async eliminarGasto(
    ...
    try {
      ...
      const result = await executeNonQuery(
        "UPDATE Gastos SET activo = 0, actualizado_en = CURRENT_TIMESTAMP WHERE id = ?",
        [id],
      );
      ...
      if (
        ...
        await executeNonQuery(
          `UPDATE Gastos SET activo = 0, actualizado_en = CURRENT_TIMESTAMP 
  ...
  static async obtenerEstadisticasGastos(
    ...
    try {
      ...
      switch (periodo) {
        case "hoy":
          query += ` AND DATE(fecha_gasto) = DATE('now', 'localtime')`;
          break;
        case "semana":
          query += ` AND fecha_gasto >= DATE('now', '-7 days', 'localtime') AND fecha_gasto <= DATE('now', 'localtime')`;
          break;
        case "mes":
          query += ` AND strftime('%Y-%m', fecha_gasto) = strftime('%Y-%m', 'now', 'localtime')`;
          break;
  ...
  > {
    try {
      const query = `
        ...
        FROM Gastos 
        WHERE punto_id = ? AND activo = 1
          AND strftime('%Y-%m', fecha_gasto) = strftime('%Y-%m', 'now')
      ...
      if (
        ...
        if (camposValidos.length > 0) {
          const query = `UPDATE Gastos SET ${camposValidos
            .map((campo) => `${campo} = ?`)
            .join(", ")}, actualizado_en = CURRENT_TIMESTAMP WHERE id = ?`;

          await executeNonQuery(query, [...valoresValidos, id]);
        }
